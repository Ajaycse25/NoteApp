...
export default function SignInPage() {
  const { requestOtpMutation, loginMutation } = useAuth();
  const {
    register,
    handleSubmit,
    getValues,
    ...
    <div className="min-h-screen flex">
      <div className="flex-1 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-md space-y-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Timer display */}
            {timer > 0 && (
              <p className="text-center text-sm text-gray-600">Request OTP again in {formatTimer(timer)}</p>
            )}
          ...
          <div className="space-y-4">
            <Button
              ...
              className="w-full border border-gray-300 hover:bg-gray-50 text-gray-700 py-2 px-4 rounded-lg font-medium flex items-center justify-center space-x-2"
            >
      ...
      <div className="hidden lg:flex flex-1 relative">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-600 to-blue-800">
          <img src="authImg.jpg" alt="Abstract blue wave background" className="object-cover w-full h-full" />
        </div>
      </div>
    </div>
  );
